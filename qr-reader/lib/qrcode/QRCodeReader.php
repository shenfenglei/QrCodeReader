...
class QRCodeReader implements Reader {
    ...
    // @Override
    public function getLocation($image){/* Map<DecodeHintType,?> hints*/
        $data = [];
        $decoderResult = null;
        $points = array();
        $detector = new Detector($image->getBlackMatrix());
        $detectorResult = $detector->local();
        if($detectorResult){
            $detectorResult = $this->object_array($detectorResult);
            foreach ($detectorResult as $key1=>$arr1){
                foreach ($arr1 as $key2=>$val){
                    $data[trim(str_replace(['Zxing\Qrcode\Detector\FinderPatternInfo','Zxing\Qrcode\Detector\FinderPattern','Zxing\ResultPoint'],[''],$key1))][trim(str_replace(['Zxing\Qrcode\Detector\FinderPatternInfo','Zxing\Qrcode\Detector\FinderPattern','Zxing\ResultPoint'],[''],$key2))] = $val;
                }
            }
        }
//        if(isset()){
